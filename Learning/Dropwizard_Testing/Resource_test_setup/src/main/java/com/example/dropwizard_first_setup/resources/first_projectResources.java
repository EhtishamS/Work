...
public class first_projectResources {
    ...
    public first_projectResources(MyDAO dao) {
        this.dao = dao;
    }
    
    @GET
    public ArrayList<Anagrafica> getAllAnagrafe(){
        ArrayList<Anagrafica> anagrafe = dao.getAllAnagrafica();
        
        for(Anagrafica x : anagrafe){
            x.setAddresses(getAllIndirizzoAnagrafeByID(x.getID()));
        }
