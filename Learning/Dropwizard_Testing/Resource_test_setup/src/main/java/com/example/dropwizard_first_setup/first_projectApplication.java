...
import com.example.dropwizard_first_setup.resources.RabbitResource;
...
public class first_projectApplication extends Application<first_projectConfiguration> {
    ...
    public void run(final first_projectConfiguration configuration, final Environment environment) {
        ...
        first_projectResources resource = new first_projectResources(dao);
        environment.jersey().register(resource);
        
        RabbitResource rbResource = new RabbitResource(sender);
        environment.jersey().register(rbResource);
    }
